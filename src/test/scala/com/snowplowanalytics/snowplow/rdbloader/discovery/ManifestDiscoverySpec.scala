...
class ManifestDiscoverySpec extends Specification { def is = s2"""
  ...
  val loaderApp13WithId = Application(Agent("snowplow-rdb-loader", "0.13.0"), Some("id"))
...
object ManifestDiscoverySpec {
  ...
  def interpreter(records: List[Record]): LoaderA ~> Id = new (LoaderA ~> Id) {
    ...
    def apply[A](effect: LoaderA[A]): Id[A] = {
      effect match {
        case LoaderA.ManifestDiscover(application, predicate) =>
  ...
  case class InMemoryManifest(records: List[Record]) extends ProcessingManifest[F](SpecHelpers.resolver) {
    ...
    def put(id: ItemId, app: Application, previousId: Option[UUID], state: State, agent: Option[Agent], payload: Option[SelfDescribingData[Json]]): Either[ManifestError, (UUID, Instant)] =
      Right {
        val time = Instant.now()
