...
import com.snowplowanalytics.snowplow.rdbloader.TestInterpreter.{AWSResults, JDBCResults, ControlResults, TestState}
...
import com.snowplowanalytics.snowplow.rdbloader.TestInterpreter.Test
...
class RedshiftLoaderSpec extends Specification { def is = s2"""
  ...
  def e6 = {
    def executeQuery[A](query: SqlString)(implicit ev: Decoder[A]): LoaderAction[Test, A] = {
      val result: Option[Any] = if (query.contains("FROM atomic.events")) {
        ...
        Some(Entities.LoadManifestItem(etlTime, commitTime, 1000, 5))
    ...
    val expected = List(
      ...
      "SELECT * FROM atomic.manifest WHERE etl_tstamp = '2018-02-28 01:50:41.133' ORDER BY etl_tstamp DESC LIMIT 1"
    )
  ...
  def e7 = {
    ...
    val expected = List(
      ...
      "SELECT * FROM atomic.manifest WHERE etl_tstamp = '1970-01-18 21:16:04.735' ORDER BY etl_tstamp DESC LIMIT 1",
      "COPY",
