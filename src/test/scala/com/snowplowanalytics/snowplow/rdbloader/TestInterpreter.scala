...
object TestInterpreter {
  ...
  def executeUpdate(query: String): Test[Either[LoaderError, Long]] =
    State { log => (trim(query) :: log, Right(1L)) }
  ...
  def getSchemas(vendor: String, name: String, model: Int): Test[Either[LoaderError, OrderedSchemas]] =
    SchemaList
      .parseStrings(List(s"iglu:$vendor/$name/jsonschema/$model-0-0"))
      .map { x => OrderedSchemas.fromSchemaList(x, TestInterpreter.fetch).value }
      .sequence[Test, Either[String, OrderedSchemas]]
