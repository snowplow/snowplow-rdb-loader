...
object ManifestDiscovery {
  ...
  def discover(id: String, region: String, jsonpathAssets: Option[S3.Folder]): LoaderAction[List[DataDiscovery]] = {
    val itemsA = LoaderA.manifestDiscover(getLoaderApp(id), shredded)
  ...
  def discoverFolder(folder: S3.Folder,
    ...
    val itemA: ActionE[Item] = LoaderA.manifestDiscover(getLoaderApp(id), folderPredicate(id, folder)).map {
  ...
  def shredded(item: Item): Boolean =
    Item.processedBy(ShredderApp, item)
  ...
  def folderPredicate(id: String, folder: S3.Folder)(item: Item): Boolean =
    Item.processedBy(ShredderApp, item) && item.id === folder
