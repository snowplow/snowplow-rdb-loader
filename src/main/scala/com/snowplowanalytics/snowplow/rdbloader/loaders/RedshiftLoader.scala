...
object RedshiftLoader {
  ...
  def loadFolder(steps: Set[Step])(statements: RedshiftLoadStatements): LoaderAction[Unit] = {
    ...
    def loadTransaction = for {
      empty <- getLoad(checkManifest, statements.dbSchema, statements.atomicCopy, statements.discovery.atomicSize)
      _ <- executeUpdates(statements.shredded)
      _ <- if (loadManifest && !empty) executeUpdate(statements.manifest)
  ...
  /**
    ...
    * @return
    */
  def getLoad(checkManifest: Boolean, dbSchema: String, copy: AtomicCopy, atomicSize: Option[Long]): LoaderAction[Boolean] = {
    def check(eventsTable: EventsTable): LoaderAction[Boolean] =
      if (checkManifest) checkLoadManifest(dbSchema, eventsTable, atomicSize) else LoaderAction.lift(false)
  ...
  def analyze(statements: RedshiftLoadStatements): LoaderAction[Unit] =
    statements.analyze match {
      case Some(analyze) =>
        for {
          _ <- LoaderA.print("ANALYZE transaction").liftA
          _ <- executeTransaction(analyze)
        } yield ()
      case None => LoaderA.print("ANALYZE skipped").liftA
    }
  ...
  def vacuum(statements: RedshiftLoadStatements): LoaderAction[Unit] = {
    statements.vacuum match {
      ...
      case None => LoaderA.print("VACUUM skipped").liftA
    }
  }
