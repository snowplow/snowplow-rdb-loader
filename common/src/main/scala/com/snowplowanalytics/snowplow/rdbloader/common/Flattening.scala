...
object Flattening {
  ...
  object FlatteningError {
    final case class SchemaListResolution(error: ClientError.ResolutionError, criterion: SchemaCriterion) extends FlatteningError
    final case class SchemaResolution(error: ClientError.ResolutionError, schema: SchemaKey) extends FlatteningError
    ...
    implicit val flatteningErrorCirceEncoder: Encoder[FlatteningError] =
      Encoder.instance {
        case SchemaListResolution(error, criterion) =>
          json"""{"error": ${error: ClientError}, "criterion": ${criterion.asString}, "type": "SchemaListResolution"}"""
        case SchemaResolution(error, schema) =>
          json"""{"error": ${error: ClientError}, "schema": ${schema.toSchemaUri}, "type": "SchemaResolution"}"""
        case Parsing(error) =>
          json"""{"error": $error, "type": "Parsing"}"""
      }
  }
