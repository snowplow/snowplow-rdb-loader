...
sealed trait Transformer[T] extends Product with Serializable {
  ...
  def sink(sc: SparkSession, compression: ShredderConfig.Compression, transformed: RDD[Transformed], outFolder: String): Unit
}
...
object Transformer {
  case class ShredTransformer(igluConfig: Json,
    ...
    def sink(spark: SparkSession, compression: ShredderConfig.Compression, transformed: RDD[Transformed], outFolder: String): Unit =
      Sink.writeShredded(spark, compression, transformed.flatMap(_.shredded), outFolder)
  }

  case class WideRowTransformer(wideRowFormat: ShredderConfig.Formats.WideRow) extends Transformer[TypesInfo.WideRow.Type] {
    ...
    def typesInfo: TypesInfo = {
      val fileFormat = wideRowFormat match {
        case ShredderConfig.Formats.WideRow.JSON => TypesInfo.WideRow.WideRowFormat.JSON
      }
