...
object EventUtils {
  ...
  def loadAndShred(client: Client[Id, Json], line: String): Either[BadRow, Event] =
    for {
      ...
      _     <- EventUtils.validateEntities(client, event)
    } yield event

  /** Build a map of columnName -> maxLength, according to `schema`. Non-string values are not present in the map */
  def getAtomicLengths(iglu: Json): Either[RuntimeException, Map[String, Int]] =
    singleton.IgluSingleton
      ...
      .leftMap(error => new RuntimeException(error))
  ...
  def validationBadRow(event: Event)(errors: NonEmptyList[(SchemaKey, ClientError)]): BadRow = {
    ...
    BadRow.LoaderIgluError(BadRowsProcessor, failure, payload)
  }

  def shreddingBadRow(event: Event)(errors: NonEmptyList[FailureDetails.LoaderIgluError]): BadRow = {
    val failure = Failure.LoaderIgluErrors(errors)
    val payload = Payload.LoaderPayload(event)
    BadRow.LoaderIgluError(BadRowsProcessor, failure, payload)
  }

  def parsingBadRow(line: String)(error: ParsingError): BadRow =
    BadRow.LoaderParsingError(BadRowsProcessor, error, Payload.RawPayload(line))
