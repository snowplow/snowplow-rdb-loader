...
class ShredJob(@transient val spark: SparkSession,
  ...
  def run(folderName: String,
    ...
    val result: Deduplication.Result = Deduplication.sytheticDeduplication(shredderConfig.deduplication, good)
    ...
    // Final output
    Sink.writeShredded(spark, shredderConfig.output.compression, shredded, outFolder)
...
object ShredJob {
  ...
  def run(
    ...
    val s3Client = Discovery.createS3Client(config.region)
