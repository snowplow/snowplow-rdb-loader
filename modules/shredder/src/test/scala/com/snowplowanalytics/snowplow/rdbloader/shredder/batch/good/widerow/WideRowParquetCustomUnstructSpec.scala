...
class WideRowParquetCustomUnstructSpec extends Specification with ShredJobSpec {
  ...
  "A job which is configured for wide row parquet output" should {
    ...
    runShredJob(events = ResourceFile("/widerow/parquet/input-events-custom-unstruct"), wideRow = Some(WideRow.Parquet))
