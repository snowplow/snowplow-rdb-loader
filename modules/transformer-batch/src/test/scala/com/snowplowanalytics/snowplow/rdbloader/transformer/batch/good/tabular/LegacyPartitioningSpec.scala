...
class LegacyPartitioningSpec extends Specification with ShredJobSpec {
  override def appName = "legacy-partition"

  sequential
  "A job which is provided with an enabled legacy partition flag" should {
    runShredJob(LegacyPartitioningSpec.lines, tsv = true, legacyPartitioning = true)
    val expectedFiles = scala.collection.mutable.ArrayBuffer.empty[String]

    "transform the enriched event and store it in legacy atomic events folder" in {
      val Some((lines, f)) = readPartFile(dirs.goodRows, LegacyAtomicFolder)
      expectedFiles += f
      lines mustEqual Seq(LegacyPartitioningSpec.expected.event)
    }
    "shred the page_context TSV into legacy path" in {
      val Some((lines, f)) = readPartFile(dirs.goodRows, LegacyPartitioningSpec.expected.contextPath)
      expectedFiles += f
      lines mustEqual Seq(LegacyPartitioningSpec.expected.contextContents)
    }
    "shred the application_error TSV into its appropriate path" in {
      val Some((lines, f)) = readPartFile(dirs.goodRows, LegacyPartitioningSpec.expected.eventPath)
      expectedFiles += f
