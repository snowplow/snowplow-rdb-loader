...
abstract class GoodDataIteratorSpec extends Specification {
  ...
  def assert(
    ...
    val badSink = new OnlyGoodDataIteratorSpec.TestBadrowsSink
    val iterator = new GoodOnlyIterator(partition.buffered, partitionIndex = 1, badrowSink = badSink, bufferMaxSize)
...
object OnlyGoodDataIteratorSpec {
  ...
  sealed trait DataType {
    def x(repeated: Int) = Data(this, repeated)
  }
  case object Good extends DataType
  case object Bad extends DataType

  final case class Data(`type`: DataType, repeated: Int)
  ...
  final class PartitionSimulation(
    ...
    private var repeatationCount: Int = 1
    private var remaining: List[Data] = initialData
    ...
    private def handleState(): Unit =
      if (repeatationCount == remaining.head.repeated) {
        moveToNextType()
      } else {
        repeatNext()
      }

    private def repeatNext(): Unit =
      repeatationCount = repeatationCount + 1

    private def moveToNextType(): Unit = {
