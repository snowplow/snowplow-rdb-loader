...
object Transformer {
  case class ShredTransformer(igluConfig: Json,
    ...
    def goodTransform(event: Event, eventsCounter: LongAccumulator): List[Transformed] =
      Transformed.shredEvent[Id](IgluSingleton.get(igluConfig), PropertiesLookupSingleton.get, isTabular, atomicLengths, ShredJob.BadRowsProcessor)(event).value match {
        case Right(shredded) =>
