      ...
      case tgt: StorageTarget.Databricks =>
        val result = new Target {
          def updateTable(migration: Migration): Block =
            Block(Nil, Nil, tgt.schema, SchemaKey(migration.vendor, migration.name, "jsonschema", migration.to))
          ...
          def extendTable(info: ShreddedType.Info): Block =
            Block(
              ...
              Nil,
              tgt.schema,
              SchemaKey(info.vendor, info.name, "jsonschema", SchemaVer.Full(info.model, 0, 0))
            )
          ...
          def createTable(schemas: SchemaList): Block = Block(Nil, Nil, tgt.schema, schemas.latest.schemaKey)
