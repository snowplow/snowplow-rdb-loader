        ...
        val result = new Target {
          ...
          override def toFragment(statement: Statement): Fragment =
            statement match {
              ...
              case Statement.VacuumEvents => sql"OPTIMIZE ${Fragment.const0(qualify(Manifest.Name))} ZORDER BY collector_tstamp"
              case Statement.VacuumManifest => sql"OPTIMIZE ${Fragment.const0(qualify(EventsTable.MainName))} ZORDER BY base"
            }
