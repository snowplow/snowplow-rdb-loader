...
object S3 {
   ...
   */
  def getClient[F[_] : ConcurrentEffect](region: String): Resource[F, S3Store[F]] =
    Resource.eval(S3Store(S3AsyncClient.builder().region(Region.of(region)).build()))

  def blobStorage[F[_] : ConcurrentEffect : Timer](client: S3Store[F]): Resource[F, BlobStorage[F]] =
