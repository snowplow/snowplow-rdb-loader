...
import com.snowplowanalytics.snowplow.rdbloader.core.algebras.Monitoring.AlertPayload.Severity
import com.snowplowanalytics.snowplow.rdbloader.generated.BuildInfo
import com.snowplowanalytics.snowplow.rdbloader.redshift.db._
import com.snowplowanalytics.snowplow.rdbloader.test.{Pure, PureAWS, PureJDBC, TestState}
...
class FolderMonitoringSpec extends Specification {
  ...
  "check" should {
    "return a single element returned by MINUS statement (shredding_complete doesn't exist)" in {
      ...
      val tableOps = TempTableOps(
        CreateAlertingTempTable,
        DropAlertingTempTable,
        FoldersCopy(loadFrom, "arn:aws:iam::123456789876:role/RedshiftLoadRole"),
        FoldersMinusManifest("atomic")
      )

      val expectedState = TestState(
        List(
          TestState.LogEntry.Sql(tableOps.minus),
          TestState.LogEntry.Sql(tableOps.copy),
          TestState.LogEntry.Sql(tableOps.create),
          TestState.LogEntry.Sql(tableOps.drop)
        ),
        Map()
      )
      val ExpectedResult = List(
        Monitoring.AlertPayload(
          BuildInfo.version,
          S3.Folder.coerce("s3://bucket/shredded/run=2021-07-09-12-30-00/"),
          Severity.Warning,
      ...
      val (state, result) = FolderMonitoring.check[Pure](tableOps).run
    ...
    "produce new keys with interval" in {
      ...
      val result = FolderMonitoring
        .getOutputKeys[IO](
          Config.Folders(
            1.second,
            S3.Folder.coerce("s3://acme/logs/"),
            None,
            S3.Folder.coerce("s3://acme/shredder-output/")
          )
        )
        .take(2)
        .compile
  ...
  "isRecent" should {
    ...
    "return false if key is old enough" in {
      val duration = FiniteDuration.apply(1, "day")
      val input =
        S3.Folder.parse("s3://bucket/run=2020-09-01-00-00-00/").getOrElse(throw new RuntimeException("Wrong key"))
      val result = FolderMonitoring.isRecent(Some(duration), Instant.now())(input)
      result must beFalse
    }
    ...
    "return true if key is fresh enough" in {
      ...
      val input =
        S3.Folder.parse("s3://bucket/run=2021-10-30-00-00-00/").getOrElse(throw new RuntimeException("Wrong key"))
...
object FolderMonitoringSpec {
  def jdbcResults(state: TestState)(statement: SqlStatement): Any = {
    val _ = state
    statement match {
