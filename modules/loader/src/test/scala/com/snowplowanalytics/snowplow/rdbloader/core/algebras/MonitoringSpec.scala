...
class MonitoringSpec extends Specification {
  "AlertPayload" should {
    "be valid against its schema" in {
      ...
      val json =
        EntityEncoder[IO, AlertPayload].toEntity(payload).body.through(utf8Decode).compile.string.map { string =>
          parse(string).flatMap(_.as[SelfDescribingData[Json]]).leftMap(_.show)
        }

      val result = EitherT(json).flatMap(data => Client.IgluCentral.check(data).leftMap(_.show)).value.unsafeRunSync()
