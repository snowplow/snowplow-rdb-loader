...
object PureMigrationBuilder {
  def interpreter: MigrationBuilder[Pure] = new MigrationBuilder[Pure] {
    override def build(items: List[MigrationBuilder.MigrationItem]): LoaderAction[Pure, MigrationBuilder.Migration[Pure]] =
      Pure
          ...
          MigrationBuilder
            .Migration[Pure](
              Pure.log(s"premigration ${items.map {
                case MigrationBuilder.MigrationItem(_, SchemaList.Full(schemas)) => schemas.map(_.self.schemaKey.toSchemaUri).toList.mkString(", ")
