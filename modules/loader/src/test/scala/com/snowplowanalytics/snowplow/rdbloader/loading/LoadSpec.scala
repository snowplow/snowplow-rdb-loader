  ...
  "load" should {
    ...
    "not retry post-load actions (VACUUM and ANALYZE) if failed" in {
      ...
      val expected = List(
        ...
        LogEntry.Message("Post-loading actions failed, skipping. Database error: Vacuum failed")
      )
      val configWithPostLoad = SpecHelpers.validConfig.copy(steps = Set(Step.Vacuum))
      val result = Load.load[Pure](SpecHelpers.validCliConfig.copy(config = configWithPostLoad), message).runS
