...
import com.snowplowanalytics.snowplow.rdbloader.core.algebras.{JDBC, Logging}
import com.snowplowanalytics.snowplow.rdbloader.test.TestState.LogEntry
import com.snowplowanalytics.snowplow.rdbloader.test.{Pure, PureJDBC, PureLogging}
...
class RedshiftLoaderSpec extends Specification {
      ...
      val input = RedshiftStatements(
        ...
        List(
          Statement.ShreddedCopy(
            "atomic",
            ShreddedType
              .Tabular(ShreddedType.Info("s3://bucket/path/run=1/".dir, "com.acme", "context", 1, Semver(1, 0, 0))),
            "eu-central-1",
            1,
            "role",
            Compression.None
  ...
  "run" should {
    "perform insertions, VACUUM and ANALYZE" >> {
      ...
      val (state, result) = RedshiftLoader
        .run[Pure](SpecHelpers.validConfig.copy(steps = Set(Step.Vacuum, Step.Analyze)), discovery)
        .flatMap(identity)
        .run
      ...
      val expected = List(
        ...
        LogEntry.Sql(
          Statement.ShreddedCopy(
            "atomic",
            ShreddedType
              .Tabular(ShreddedType.Info("s3://bucket/path/run=1/".dir, "com.acme", "context", 3, Semver(1, 5, 0))),
            "us-east-1",
            10,
            "arn:aws:iam::123456789876:role/RedshiftLoadRole",
            Compression.Gzip
