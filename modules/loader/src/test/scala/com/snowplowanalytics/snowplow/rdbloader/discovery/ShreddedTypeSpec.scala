...
class ShreddedTypeSpec extends Specification with ScalaCheck {
  "transformPath" should {
    ...
    "transform correct S3 path for Shred job" >> {
      ...
      val expected = (TypesInfo.Shredded.ShreddedFormat.JSON, Info(S3.Folder.coerce("s3://rdb-test"), "com.snowplowanalytics.snowplow", "submit_form", 1, Semver(0,13,0), LoaderMessage.SnowplowEntity.SelfDescribingEvent))
    ...
    "transform full modern shredded key" >> {
      ...
      val expected = (TypesInfo.Shredded.ShreddedFormat.JSON, Info(expectedPrefix, "com.snowplowanalytics.snowplow", "submit_form", 1, Semver(0,13,0), LoaderMessage.SnowplowEntity.SelfDescribingEvent))
    ...
    "transform correct tabular S3 path" >> {
      ...
      val expectedPrefix = S3.Folder.coerce("s3://snowplow-shredded/good/run=2017-06-14-12-07-11/")
      val expected = (TypesInfo.Shredded.ShreddedFormat.TSV, Info(expectedPrefix, "com.snowplowanalytics.snowplow", "submit_form", 1, Semver(0,16,0), LoaderMessage.SnowplowEntity.SelfDescribingEvent))
