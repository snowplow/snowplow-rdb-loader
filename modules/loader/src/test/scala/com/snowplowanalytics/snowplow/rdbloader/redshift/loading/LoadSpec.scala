...
import com.snowplowanalytics.snowplow.rdbloader.core.db.{Manifest, Statement => SqlStatement}
import com.snowplowanalytics.snowplow.rdbloader.core.discovery.{DataDiscovery, ShreddedType}
...
import com.snowplowanalytics.snowplow.rdbloader.redshift.loading.LoadSpec._
  ...
  "load" should {
    "perform COPY statements and wrap with transaction block" in {
      ...
      val info = ShreddedType.Json(
        ShreddedType.Info("s3://shredded/base/".dir, "com.acme", "json-context", 1, Semver(0, 18, 0)),
        "s3://assets/com.acme/json_context_1.json".key
      )
      val expected = List(
        LogEntry.Sql(Statement.Begin),
        LogEntry.Sql(Statement.ManifestRead("atomic", "s3://shredded/base/".dir)),
        LogEntry.Sql(
          Statement.EventsCopy("atomic", false, "s3://shredded/base/".dir, "us-east-1", 10, arn, Compression.Gzip)
        ),
        ...
        LogEntry.Sql(Statement.ManifestUpdate("atomic", LoadSpec.dataDiscoveryWithOrigin.origin, tstamp)),
      ...
      val result = load.execute[Pure](SpecHelpers.validCliConfig.config, message).value.runS
    ...
    "abort, sleep and start transaction again if first commit failed" in {
      ...
      val expected = List(
        ...
        LogEntry.Sql(Statement.ShreddedCopy("atomic", info, "us-east-1", 10, arn, Compression.Gzip)),
        LogEntry.Sql(Statement.GetLoadTstamp("atomic", Timestamp.from(message.data.origin.timestamps.max.get))),
        LogEntry.Sql(Statement.ManifestUpdate("atomic", LoadSpec.dataDiscoveryWithOrigin.origin, tstampAbort)),
        ...
        LogEntry.Sql(Statement.ShreddedCopy("atomic", info, "us-east-1", 10, arn, Compression.Gzip)),
        LogEntry.Sql(Statement.GetLoadTstamp("atomic", Timestamp.from(message.data.origin.timestamps.max.get))),
        LogEntry.Sql(Statement.ManifestUpdate("atomic", LoadSpec.dataDiscoveryWithOrigin.origin, tstampCommit)),
        LogEntry.Sql(Statement.Commit),
        LogEntry.Message("TICK REALTIME"),
        LogEntry.Sql(Statement.Begin),
        LogEntry.Sql(Statement.Analyze("atomic.events")),
    ...
    "abort and ack the command if manifest record already exists" in {
      ...
      def getResult(s: TestState)(statement: SqlStatement): Any =
        statement match {
          case Statement.ManifestRead("atomic", Base) =>
    ...
    "not retry post-load actions (VACUUM and ANALYZE) if failed" in {
      ...
      val result             = load.execute[Pure](configWithPostLoad, message).value.runS
...
object LoadSpec {
  ...
  def isFirstCommit(sql: SqlStatement, ts: TestState) =
    sql match {
      case Statement.Commit => ts.getLog.filter(entry => entry == LogEntry.Sql(Statement.Begin)).size == 1
      case _                => false
    }

  def isVacuum(sql: SqlStatement, ts: TestState) =
    sql match {
