  ...
  "build" should {
    "build Migration with table creation for ShreddedType.Tabular" in {
      ...
      val expectedMigration = List(
        ...
        LogEntry.Sql(
          Statement
            .CommentOn(CommentOn("public.com_acme_some_context_2", "iglu:com.acme/some_context/jsonschema/2-0-0"))
        ),
    ...
    "ignore atomic schema" in {
      ...
      val expectedMigration = List(
        ...
        LogEntry.Sql(Statement.CreateTable(create)),
        LogEntry.Sql(
          Statement.CommentOn(CommentOn("public.com_acme_some_event_1", "iglu:com.acme/some_event/jsonschema/1-0-0"))
        ),
        LogEntry.Message("Table created")
      )
  ...
  "updateTable" should {
    ...
    "create a Block with in-transaction migration" in {
      ...
      val expectedResult = Migration.Block(
        List(),
        List(Migration.Item.AddColumn(alterTable, Nil)),
        "db_schema",
    ...
    "create a Block with pre-transaction migration" in {
      ...
      val expectedResult = Migration.Block(
        List(Migration.Item.AlterColumn(alterTable)),
        List(),
        "db_schema",
