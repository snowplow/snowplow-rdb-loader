...
import com.snowplowanalytics.snowplow.rdbloader.{LoaderError, TestInterpreter, LoaderAction}
...
class RedshiftLoaderSpec extends Specification { def is = s2"""
  ...
  val noDiscovery = DataDiscovery(Folder.coerce("s3://noop"), None, None, Nil)
  ...
  def e1 = {
    ...
    implicit val logging: Logging[Test] = TestInterpreter.stateControlInterpreter(ControlResults.init)
    ...
    val (_, result) =
      DataDiscovery
        .discover[Test](
          DataDiscovery.DiscoveryTarget.Global(validConfig.aws.s3.buckets.shredded.good),
          validConfig.storage.versions.rdbShredder,
          validConfig.aws.s3.region,
          validConfig.aws.s3.buckets.jsonpathAssets)
        .value
        .run(TestState.init).value

    val expected =
      List(DataDiscovery(S3.Folder.coerce("s3://snowplow-acme-storage/shredded/good/run=2017-05-22-12-20-57/"), Some(1), Some(1L), Nil))
  ...
  def e2 = {
    ...
    val result = RedshiftLoadStatements.getStatements(validConfig, validTarget, steps, discovery)
    ...
    val expected = RedshiftLoadStatements("atomic", RedshiftLoadStatements.AtomicCopy.Straight(sql(atomic)), shredded, Some(vacuum), Some(analyze), sql(manifest), discovery)
  ...
  def e6 = {
    ...
    val input = RedshiftLoadStatements(
      "atomic",
      RedshiftLoadStatements.AtomicCopy.Straight(sql("LOAD INTO atomic.events MOCK")),
  ...
  def e7 = {
    ...
    val input = RedshiftLoadStatements(
      "atomic",
      RedshiftLoadStatements.AtomicCopy.Transit(sql("COPY")),
      List(sql("LOAD INTO SHRED 1 MOCK"), sql("LOAD INTO SHRED 2 MOCK"), sql("LOAD INTO SHRED 3 MOCK")),
      Some(List(sql("VACUUM MOCK"))),   // Must be shred cardinality + 1
      Some(List(sql("ANALYZE MOCK"))),
      sql("MANIFEST INSERT MOCK"),
