...
class AlertMessageSpec extends Specification {

  "AlertMessage getString" should {
    "render a readable message" in {
      ...
      val alertMessage = AlertMessage.UnskippableLoadFailure(folder, e5)

      AlertMessage.getString(
        alertMessage
      ) must_== "Load failed and will be retried: Failed to establish a JDBC connection: Error setting up SSH tunnel: Problem with network"
    }
    ...
    "not exceed the maximum length for an alert message" in {
      ...
      val folder = BlobStorage.Folder.coerce("s3://bucket/1/")
      val alertMessage = AlertMessage.UnskippableLoadFailure(folder, e)

      AlertMessage.getString(alertMessage) must haveSize(4096)
    }
  }
}
