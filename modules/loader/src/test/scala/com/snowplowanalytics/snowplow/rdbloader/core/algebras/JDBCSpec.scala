...
import com.snowplowanalytics.snowplow.rdbloader.test.{Pure, PureLogging, TestState}
  ...
  "log" should {
    "produce a sensible error message, mentioning retries" in {
      ...
      state.getLog must beEqualTo(
        List(
          LogEntry.Message(
            "Warning. Cannot acquire connection: Something went wrong. One attempt has been made, waiting for 2 seconds until the next one"
          )
        )
      )
    }
