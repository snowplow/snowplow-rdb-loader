 ...
 */
package com.snowplowanalytics.snowplow.rdbloader.core.discovery

import cats.implicits._
import com.snowplowanalytics.snowplow.rdbloader.core.algebras.{AWS, Cache}
import org.scalacheck.Gen
import org.specs2.ScalaCheck
import org.specs2.mutable.Specification

// This project
import com.snowplowanalytics.snowplow.rdbloader.core.discovery.ShreddedType._
...
class ShreddedTypeSpec extends Specification with ScalaCheck {
  "transformPath" should {
    "fail to transform path without valid vendor" >> {
      val path =
        "cross-batch-test/shredded-archive/run%3D2017-04-27-14-39-42/submit_form/jsonschema/1-0-0/part-00000-00001"
    ...
    "fail to transform path without file" >> {
      val path =
        "cross-batch-test/shredded-archive/run%3D2017-04-27-14-39-42/com.snowplowanalytics.snowplow/submit_form/jsonschema/1-0-0"
      val key    = S3.Key.coerce(s"s3://rdb-test/$path")
    ...
    "transform correct S3 path for Shred job" >> {
      ...
      val expected = (
        ...
        Info(S3.Folder.coerce("s3://rdb-test"), "com.snowplowanalytics.snowplow", "submit_form", 1, Semver(0, 13, 0))
      )
    ...
    "transform full modern shredded key" >> {
      val key = S3
        .Key
        .coerce(
          "s3://snowplow-shredded/good/run=2017-06-14-12-07-11/vendor=com.snowplowanalytics.snowplow/name=submit_form/format=json/model=1/part-00000-00001"
        )

      val expectedPrefix = S3.Folder.coerce("s3://snowplow-shredded/good/run=2017-06-14-12-07-11/")
      val expected = (
        LoaderMessage.Format.JSON,
        Info(expectedPrefix, "com.snowplowanalytics.snowplow", "submit_form", 1, Semver(0, 13, 0))
      )
    ...
    "transform correct tabular S3 path" >> {
      val key = S3
        .Key
        .coerce(
          "s3://snowplow-shredded/good/run=2017-06-14-12-07-11/vendor=com.snowplowanalytics.snowplow/name=submit_form/format=tsv/model=1/part-00000-00001"
        )

      val expectedPrefix = S3.Folder.coerce("s3://snowplow-shredded/good/run=2017-06-14-12-07-11/")
      val expected = (
        LoaderMessage.Format.TSV,
        Info(expectedPrefix, "com.snowplowanalytics.snowplow", "submit_form", 1, Semver(0, 16, 0))
      )
  ...
  "getSnowplowJsonPath" should {
    "put a result into cache only once" >> {
      ...
      state.getLog must beEqualTo(
        List(
          LogEntry.Message(
            "PUT foo_1.json: Some(s3://snowplow-hosted-assets-eu-central-1/4-storage/redshift-storage/jsonpaths/foo_1.json)"
          )
        )
      )
    }
...
object ShreddedTypeSpec {
  ...
  implicit val alphaNum: Gen[String] = for {
    n <- Gen.chooseNum(1, 5)
    d <- Gen.oneOf('_', '.', '-')
    s <- Gen.listOf(Gen.alphaNumChar).map(_.mkString).suchThat(_.nonEmpty)
    (a, b) = s.splitAt(n)
    r <- Gen.const(s"$a$d$b").suchThat(x => !x.startsWith(d.toString)).suchThat(x => !x.endsWith(d.toString))
  } yield r
