...
class FolderMonitoringSpec extends Specification {
  ...
  "check" should {
    "return a single element returned by MINUS statement (shredding_complete doesn't exist)" in {
      ...
      val expectedState = TestState(List(
        TestState.LogEntry.Sql(Statement.FoldersMinusManifest("atomic", loadFrom)),
        ...
        TestState.LogEntry.Sql(Statement.CreateAlertingTempTable)),Map()
      )
      val ExpectedResult = List(
        Monitoring.AlertPayload(BuildInfo.version, S3.Folder.coerce("s3://bucket/shredded/run=2021-07-09-12-30-00/"), Severity.Warning, "Corrupted batch", Map.empty)
      )
  ...
  "getOutputKey" should {
    "produce new keys with interval" in {
      ...
      result.forall(_.endsWith("/keys")) must beTrue
    }
  }
}
...
object FolderMonitoringSpec {
  def jdbcResults(state: TestState)(statement: Statement): Any = {
    val _ = state
    statement match {
      case Statement.FoldersMinusManifest(_, _) =>
