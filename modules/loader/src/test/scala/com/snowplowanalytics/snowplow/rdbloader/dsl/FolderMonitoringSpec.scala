...
class FolderMonitoringSpec extends Specification {
  "check" should {
    "return a single element returned by MINUS statement (shredding_complete doesn't exist)" in {
      implicit def aws: AWS[Pure] = PureAWS.interpreter(PureAWS.init)

      val loadFrom = S3.Folder.coerce("s3://bucket/shredded/")
      val expectedState = TestState(
        List(
          PureTransaction.NoTransactionMessage,
          LogEntry.Message("foldersMinusManifest"),
          PureTransaction.NoTransactionMessage,
          PureTransaction.CommitMessage,
          LogEntry.Message("foldersCopy s3://bucket/shredded/"),
          LogEntry.Message("createAlertingTempTable"),
          LogEntry.Message("dropAlertingTempTable"),
      ...
      val ExpectedResult = List(
        Monitoring.AlertPayload(
          ...
          Some(S3.Folder.coerce("s3://bucket/shredded/run=2021-07-09-12-30-00/")),
          Severity.Warning,
