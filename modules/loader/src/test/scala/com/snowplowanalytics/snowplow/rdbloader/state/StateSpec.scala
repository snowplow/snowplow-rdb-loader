...
class StateSpec extends Specification {
  "showExtended" should {
    "provide well-formatted string" in {
      val folder = S3.Folder.coerce("s3://snowplow/example/")
      val state = State(Load.Status.Loading(folder, Stage.MigrationBuild), Instant.ofEpochMilli(1640000000000L), 1, Map.empty, 10, 10)
      val expected = "Total 10 messages received, 10 loaded; Loader is in Ongoing processing of s3://snowplow/example/ at migration building state; 1 attempts has been made to load current folder; Last state update at 2021-12-20 11:33:20.000"
