...
import com.snowplowanalytics.snowplow.rdbloader.redshift.loading.RedshiftStatements._
...
object RedshiftLoader {
  ...
  /**
    ...
    */
  def run[F[_]: Monad: Logging: JDBC](config: Config[StorageTarget.Redshift], discovery: DataDiscovery) =
    for {
  ...
  def analyze[F[_]: Monad: Logging: JDBC](statements: RedshiftStatements): LoaderAction[F, Unit] =
    statements.analyze match {
      case Some(analyze) =>
        for {
          _ <- JDBC[F].executeTransaction(analyze, Statement.Begin, Statement.Commit)
