...
object JDBC {
  ...
  def jdbcRealInterpreter[F[_]: Logging: Sync](conn: Transactor[F]): JDBC[F] = new JDBC[F] {
    /**
    ...
    def executeUpdate(sql: Statement): LoaderAction[F, Int] = {
      val update = sql
        ...
        .flatMap[Either[LoaderError, Int]] {
          ...
          case Left(e) =>
            Logging[F].error(e, "RDB Loader unknown error in executeUpdate")
    ...
    def query[G[_], A](get: Query0[A] => ConnectionIO[G[A]], sql: Query0[A]): F[Either[LoaderError, G[A]]] =
      get(sql)
        ...
        .flatMap[Either[LoaderError, G[A]]] {
          case Left(e) =>
            Logging[F].error(e, "RDB Loader unknown error in executeQuery")
