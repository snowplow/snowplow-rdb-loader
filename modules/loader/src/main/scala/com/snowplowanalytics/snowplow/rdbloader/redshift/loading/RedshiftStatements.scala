...
object RedshiftStatements {
  ...
  private[loading] def getStatements(config: Config[Redshift], discovery: DataDiscovery): RedshiftStatements = {
    val shreddedStatements =
      discovery.shreddedTypes.filterNot(_.isAtomic).map(transformShreddedType(config, discovery.compression))
    ...
    val atomic = Statement.EventsCopy(
      ...
      discovery.base,
      config.region.name,
      config.storage.maxError,
      config.storage.roleArn,
  ...
  /**
    ...
    */
  private case class ShreddedStatements(
    copy: Statement.ShreddedCopy,
  ...
  /**
    ...
    */
  private def transformShreddedType(config: Config[Redshift], compression: Compression)(
    shreddedType: ShreddedType
  ): ShreddedStatements = {
    val tableName = config.storage.shreddedTable(shreddedType.getTableName)
    val copyFromJson = Statement.ShreddedCopy(
      config.storage.schema,
      shreddedType,
      config.region.name,
      config.storage.maxError,
