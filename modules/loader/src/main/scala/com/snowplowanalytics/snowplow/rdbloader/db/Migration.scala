    ...
    description match {
      case Description.Table(schemas) =>
        ...
        val createMissingRecoveryTables: F[List[Block]] = schemaKeysWithModels.values
          ...
          .toList
          .traverseFilter(rm =>
            Control.tableExists[F](rm.tableName).ifM(Applicative[F].pure(rm.some), Applicative[F].pure(Option.empty[RecoveryModel]))
          )
        ...
        optUpdateGoodTable.flatMap(_.fold(createTables)(ugt => createTables.map(_ ::: List(ugt))))
