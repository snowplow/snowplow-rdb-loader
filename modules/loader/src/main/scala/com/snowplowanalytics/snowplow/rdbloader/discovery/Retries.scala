...
object Retries {
  ...
  final case class LoadFailure(lastError: Throwable, attempts: Int, first: Instant, last: Instant) {
  ...
  /**
    ...
    */
  def run[F[_]: AWS: Cache: Logging: Timer: Concurrent](
    region: String,
  ...
  def get[F[_]: AWS: Cache: Logging: Timer: MonadThrow](
    region: String,
    assets: Option[S3.Folder],
  ...
  def fromLoaderMessage[F[_]: Monad: AWS: Cache: Logging](region: String, assets: Option[S3.Folder])(
    ...
    DataDiscovery.fromLoaderMessage[F](region, assets, message).value.flatMap {
      case _ if DataDiscovery.isEmpty(message) =>
        Logging[F]
          .warning(
            s"Empty folder ${message.base} re-discovered in failure manifest. It might signal about corrupted S3"
          )
          .as(none)
  ...
  def addFailure[F[_]: Clock: Monad](
    config: Config.RetryQueue,
    state: State.Ref[F]
  )(base: S3.Folder, error: Throwable): F[Boolean] =
  ...
  def readShreddingComplete[F[_]: AWS: MonadThrow](folder: S3.Folder): F[LoaderMessage.ShreddingComplete] = {
    ...
    AWS[F].readKey(fullPath).flatMap {
      case Some(content) =>
