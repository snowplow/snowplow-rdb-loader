...
trait LoadAuthService[F[_]] {
  def getLoadAuthMethod: F[LoadAuthService.LoadAuthMethod]
}
...
object LoadAuthService {
   ...
   */
  def aws[F[_]: Concurrent: ContextShift](authMethodConfig: StorageTarget.LoadAuthMethod,
                                          region: String,
    ...
    for {
      ...
      authService = new LoadAuthService[F] {
        override def getLoadAuthMethod: F[LoadAuthMethod] =
  ...
  def noop[F[_]: Concurrent]: Resource[F, LoadAuthService[F]] =
    Resource.pure[F, LoadAuthService[F]](new LoadAuthService[F] {
      override def getLoadAuthMethod: F[LoadAuthMethod] = Concurrent[F].pure(LoadAuthMethod.NoCreds)
    })
}
