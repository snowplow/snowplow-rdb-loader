...
object StatsDReporter {
  ...
  private def sendMetric[F[_]: ContextShift: Sync](
    ...
    Sync[F].delay(println(s"Sending $metric to statsd located on ${addr.getHostName()}-${addr.getHostAddress()}:$port")) >> blocker.delay(socket.send(packet))
  }
  ...
  private def statsDFormat(config: Config.StatsD)(metric: Metrics.KVMetric): String = {
    ...
    val prefix = config.prefix.getOrElse(Config.DefaultPrefix).stripSuffix(".")
