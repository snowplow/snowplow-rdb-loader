...
object Retry {
  ...
  /**
    ...
    */
  def retryLoad[F[_]: MonadThrow: Logging: Timer: Control, A](fa: F[A]): F[A] = {
    val onError = (e: Throwable, d: RetryDetails) => Control[F].incrementAttempts *> log[F](e, d)
  ...
  def isWorth(e: Throwable): Boolean = {
    val isFatal = FatalFailures.foldLeft(false) { (isPreviousFatal, predicate) =>
      predicate(e) || isPreviousFatal
    }
    !isFatal
  }
  ...
  val FatalFailures: List[Throwable => Boolean] = List(
    ...
    e =>
      e.toString
        .toLowerCase
        .contains("invalid operation: number of jsonpath") && e.toString.toLowerCase.contains("columns should match"),
