...
import com.snowplowanalytics.iglu.schemaddl.redshift.{
  ...
  AlterType,
  CommentOn,
  AddColumn   => DAddColumn,
  CreateTable => DCreateTable
}
...
import com.snowplowanalytics.snowplow.rdbloader.core.{ActionOps, LoaderAction, LoaderError, readSchemaKey}
import com.snowplowanalytics.snowplow.rdbloader.core.discovery.{DataDiscovery, DiscoveryFailure, ShreddedType}
import com.snowplowanalytics.snowplow.rdbloader.core.algebras.{Iglu, JDBC, Logging}
...
object Migration {
  ...
  def build[F[_]: Monad: Logging: Iglu: JDBC](
    dbSchema: String,
    discovery: DataDiscovery
  ): LoaderAction[F, Migration[F]] =
  ...
  /**
    ...
    */
  def prepare[F[_]: Monad: Logging: Iglu: JDBC](
    dbSchema: String,
    discovery: DataDiscovery
  ): LoaderAction[F, List[Option[Block]]] =
  ...
  /**
    ...
    */
  def forTabular[F[_]: Monad: Iglu: JDBC](
    dbSchema: String,
    vendor: String,
    name: String,
    model: Int
  ): LoaderAction[F, Option[Block]] =
    for {
      schemas <- EitherT(Iglu[F].getSchemas(vendor, name, model))
      tableName = StringUtils.getTableName(
        SchemaMap(SchemaKey(vendor, name, "jsonschema", SchemaVer.Full(model, 0, 0)))
      )
      block <- for {
        exists <- Utils.tableExists[F](dbSchema, tableName)
        block <- if (exists) for {
          schemaKey <- getVersion[F](dbSchema, tableName)
          matches = schemas.latest.schemaKey == schemaKey
          columns <- Utils.getColumns[F](dbSchema, tableName)
          block <- if (matches) emptyBlock[F]
          else LoaderAction.liftE[F, Block](updateTable(dbSchema, schemaKey, columns, schemas)).map(_.some)
        } yield block
        else LoaderAction.pure[F, Option[Block]](Some(createTable(dbSchema, schemas)))
      } yield block
    } yield block
  ...
  def getVersion[F[_]: Monad: JDBC](dbSchema: String, tableName: String): LoaderAction[F, SchemaKey] =
    JDBC[F]
      .executeQuery[SchemaKey](Statement.GetVersion(dbSchema, tableName))(readSchemaKey)
      .leftMap(Utils.annotateError(dbSchema, tableName))
  ...
  def updateTable(
    ...
    state match {
      ...
      case _: DSchemaList.Single =>
        val message =
          s"Illegal State: updateTable called for a table with known single schema [${current.toSchemaUri}]\ncolumns: ${columns
            .mkString(", ")}\nstate: $state"
  ...
  /**
    ...
    */
  def addColumn[F[_]: Monad: JDBC](
    schemaName: String,
    tableName: String,
    ddlAddColumn: DAddColumn
  ): LoaderAction[F, Unit] = {
    val alterTable  = AlterTable(s"$schemaName.$tableName", ddlAddColumn)
    val addColumn   = Item.AddColumn(alterTable, Nil)
    val transaction = Utils.withTransaction(JDBC[F].executeUpdate(addColumn.statement))
    for {
      columns <- Utils.getColumns[F](schemaName, tableName)
