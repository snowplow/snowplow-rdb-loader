...
object Load {
  ...
  private def congratulate[F[_]: Monad: Logging: Timer](message: LoaderMessage.ShreddingComplete): LoaderAction[F, Unit] =
    Timer[F].clock.instantNow.flatMap { now =>
      val count = message.count.map(c => s"$c events").getOrElse("volume is unknown")
      val latency = message.timestamps.min match {
        case Some(earliest) => s"${Duration.between(earliest, now).toMinutes} minutes"
        case None => "unknown"
      }
      Logging[F].info(s"Data from ${message.base} has been loaded and committed. Success!") *>
        Logging[F].info(s"Latency for ${message.base} ($count): $latency")
    }.liftA
}
