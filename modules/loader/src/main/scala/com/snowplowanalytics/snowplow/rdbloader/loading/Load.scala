...
import com.snowplowanalytics.snowplow.rdbloader.db.{Control, ManagedTransaction, Manifest, Migration, Target}
...
object Load {
   ...
   *
   * @param txnConfig
   *   Configuration for retries and target check
   * @param setStage
  ...
  def load[F[_]: MonadThrow: Logging: Iglu: Sleep: Transaction[*[_], C], C[_]: MonadThrow: Logging: LoadAuthService: DAO, I](
    ...
    for {
      migrations <- Migration.build[F, C, I](discovery.discovery, target, txnConfig)
      _ <- ManagedTransaction.run[F, C](txnConfig, "pre-transaction migrations") {
             getPreTransaction[C](setStage, migrations.preTransaction)
           }
      result <- ManagedTransaction.transact[F, C](txnConfig, show"loading batch ${discovery.discovery.base}") {
