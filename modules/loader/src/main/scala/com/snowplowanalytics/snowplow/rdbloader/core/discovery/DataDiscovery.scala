...
import com.snowplowanalytics.snowplow.rdbloader.core.{DiscoveryStep, DiscoveryStream, LoaderAction, LoaderError, State}
...
case class DataDiscovery(base: S3.Folder, shreddedTypes: List[ShreddedType], compression: Compression) {
  ...
  def show: String = {
    ...
    val shredded =
      if (shreddedTypes.isEmpty) "without shredded types" else s"with following shredded types:\n$shreddedTypesList"
    s"$runId $shredded"
  }
}
...
object DataDiscovery {
  ...
  def discover[F[_]: MonadThrow: AWS: Cache: Logging](config: Config[_], state: State.Ref[F]): DiscoveryStream[F] =
    AWS[F].readSqs(config.messageQueue).evalMapFilter { message =>
  ...
  /**
    ...
    */
  def handle[F[_]: MonadThrow: AWS: Cache: Logging](
    region: String,
    assets: Option[S3.Folder],
    message: LoaderMessage.ShreddingComplete,
    ack: F[Unit]
  ) =
    fromLoaderMessage[F](region, assets, message).value.flatMap[Option[Message[F, WithOrigin]]] {
  ...
  /**
    ...
    * @tparam F effect type to perform AWS interactions
    */
  def fromLoaderMessage[F[_]: Monad: Cache: AWS](
    region: String,
    assets: Option[S3.Folder],
    message: LoaderMessage.ShreddingComplete
  ...
  def isEmpty(message: LoaderMessage.ShreddingComplete): Boolean =
    message.timestamps.min.isEmpty && message.timestamps.max.isEmpty && message.types.isEmpty && message
      .count
      .contains(0)
}
