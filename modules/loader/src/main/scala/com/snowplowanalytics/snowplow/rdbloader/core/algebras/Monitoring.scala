...
import com.snowplowanalytics.snowplow.rdbloader.core.algebras.metrics.{Metrics, Reporter}
...
object Monitoring {
  ...
  val LoadSucceededSchema =
    SchemaKey("com.snowplowanalytics.monitoring.batch", "load_succeeded", "jsonschema", SchemaVer.Full(1, 0, 0))
  val LoadFailedSchema =
    SchemaKey("com.snowplowanalytics.monitoring.batch", "load_failed", "jsonschema", SchemaVer.Full(1, 0, 0))
        ...
        webhookConfig match {
          case Some(webhook) =>
            val request: Request[F] =
              Request[F](Method.POST, webhook.endpoint).withEntity(payload.copy(tags = payload.tags ++ webhook.tags))
  ...
  /**
    ...
    */
  def initializeTracking[F[_]: ConcurrentEffect: Timer: Clock](
  ...
  /** Callback for failed  */
  private def callback[F[_]: Sync: Clock](
