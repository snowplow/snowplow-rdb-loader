...
object RetryingTransactor {
   ...
   */
  def wrap[F[_]: MonadCancelThrow: Logging: Sleep, A](
    config: Config.Retries,
    inner: Transactor.Aux[F, A]
  ): Transactor.Aux[F, A] = {
    val policy = Retry.getRetryPolicy[F](config)
    inner.copy(connect0 = a => wrapResource(policy, inner.connect(a)))
  }

  private def wrapResource[F[_]: MonadCancelThrow: Logging: Sleep](policy: RetryPolicy[F], resource: Resource[F, Connection]) =
    Resource.suspend {
      retryingOnSomeErrors(policy, isConnectionError.andThen(_.pure[F]), onError[F](_, _)) {
        resource.use(conn => Resource.pure[F, Connection](conn).pure[F])
      }.adaptError {
  ...
  private def onError[F[_]: Logging](t: Throwable, d: RetryDetails): F[Unit] =
    Logging[F].info(show"Target is not ready. $d. ${t.getMessage}")
}
