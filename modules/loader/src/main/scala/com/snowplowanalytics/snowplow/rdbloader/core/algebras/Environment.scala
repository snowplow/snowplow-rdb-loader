...
import com.snowplowanalytics.snowplow.rdbloader.core.State
import com.snowplowanalytics.snowplow.rdbloader.core.config.CliConfig
import com.snowplowanalytics.snowplow.rdbloader.core.algebras.metrics._
import com.snowplowanalytics.snowplow.rdbloader.core.utils.SSH
...
object Environment {
  ...
  def initialize[F[_]: Clock: ConcurrentEffect: ContextShift: Timer: Parallel](
    ...
    for {
      ...
      implicit0(iglu: Iglu[F]) <- Iglu.igluInterpreter(httpClient, cli.igluConfig)
      tracker                  <- Monitoring.initializeTracking[F](cli.config.monitoring, httpClient)
      implicit0(logging: Logging[F]) = Logging.loggingInterpreter[F](
        List(cli.config.storage.password.getUnencrypted, cli.config.storage.username)
      )
