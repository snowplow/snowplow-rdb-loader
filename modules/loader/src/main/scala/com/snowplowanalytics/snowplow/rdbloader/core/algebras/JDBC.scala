...
import doobie._
import doobie.free.connection.{abort, unit, setAutoCommit => autoCommit}
...
import com.snowplowanalytics.snowplow.rdbloader.core.LoaderError.StorageTargetError
import com.snowplowanalytics.snowplow.rdbloader.core.config.StorageTarget
import com.snowplowanalytics.snowplow.rdbloader.core.db.{Pool, Statement}
import com.snowplowanalytics.snowplow.rdbloader.core.db.Statement.{BeginStatement, CommitStatement}
...
trait JDBC[F[_]] { self =>
  ...
  def executeTransaction(queries: List[Statement], begin: BeginStatement, commit: CommitStatement)(
    implicit A: Monad[F]
  ): LoaderAction[F, Unit] =
    executeUpdates((begin :: queries) :+ commit)
}
...
object JDBC {
  ...
  def retriesMessage(details: RetryDetails): String = {
    val wait = (d: Option[FiniteDuration]) =>
      d.fold("Giving up")(x => s"waiting for ${x.toSeconds} seconds until the next one")
    if (details.retriesSoFar == 0) s"One attempt has been made, ${wait(details.upcomingDelay)}"
    else
      s"${details.retriesSoFar} retries so far, ${details.cumulativeDelay.toSeconds} seconds total. ${details
        .upcomingDelay
        .fold("Giving up")(x => s"waiting for ${x.toSeconds} seconds until the next one")}"
  }

  // 2 + 4 + 8 + 16 + 32 = 62
  def retryPolicy[F[_]: Monad]: RetryPolicy[F] =
    RetryPolicies.limitRetries[F](MaxRetries).join(RetryPolicies.exponentialBackoff(Backoff))

  /** Build a `Pool` for DB connections */
  def buildPool[F[_]: Concurrent: ContextShift: Logging: Timer: AWS](
    target: StorageTarget
  ): Resource[F, Pool[F, Connection]] = {
    val acquire = for {
          ...
          r.jdbc.validation match {
            ...
            case Right(propertyUpdaters) =>
              ...
              Sync[F]
                .delay(
                  new RedshiftDriver()
                    .connect(s"jdbc:redshift://${target.host}:${target.port}/${target.database}", props)
                )
                .flatMap { conn =>
                  Sync[F].delay { conn.setAutoCommit(false); conn }
                }
                .onError {
                  case _ =>
    ...
    Pool
      .create[F, Connection](acquire, release, MaxConnections)
      .onFinalize(Logging[F].info("RDB Pool has been destroyed"))
  }
  ...
  /**
    ...
    */
  def interpreter[F[_]: Concurrent: ContextShift: Logging: Monitoring: Timer: AWS](
    target: StorageTarget,
    dryRun: Boolean,
    blocker: Blocker
  ): Resource[F, JDBC[F]] =
  ...
  def jdbcRealInterpreter[F[_]: Logging: Monitoring: Sync](conn: Transactor[F]): JDBC[F] = new JDBC[F] {
    ...
    /**
      ...
      */
    def executeUpdate(sql: Statement): LoaderAction[F, Int] = {
      val update = sql.toFragment.update.run.transact(conn).attemptSql.flatMap[Either[LoaderError, Int]] {
        case Left(e: SQLException)
            if Option(e.getMessage).getOrElse("").contains("is not authorized to assume IAM Role") =>
          (StorageTargetError("IAM Role with S3 Read permissions is not attached to Redshift instance"): LoaderError)
            .asLeft[Int]
            .pure[F]
        case Left(e) =>
    ...
    def query[G[_], A](get: Query0[A] => ConnectionIO[G[A]], sql: Query0[A]): F[Either[LoaderError, G[A]]] =
      get(sql).transact(conn).attemptSql.flatMap[Either[LoaderError, G[A]]] {
        case Left(e) =>
