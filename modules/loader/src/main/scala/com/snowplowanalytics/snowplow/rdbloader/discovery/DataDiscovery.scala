...
object DataDiscovery {
  ...
  /**
    ...
    */
  def discover[
    F[_]: MonadThrow: AWS: Cache: Logging: Control,
    T <: StorageTarget
  ](config: Config[T]): DiscoveryStream[F] =
    AWS[F].readSqs(config.messageQueue).evalMapFilter { message =>
      ...
      Logging[F].info("Received a new message") *>
        ...
        Control[F].incrementAttempts.flatMap(state => Logging[F].info(state.show)) *>
        action
    }
  ...
  def isEmpty(message: LoaderMessage.ShreddingComplete): Boolean =
    message.timestamps.min.isEmpty && message.timestamps.max.isEmpty && message.typesInfo.isEmpty && message
      .count
      .contains(LoaderMessage.Count(0))
}
