...
object Main extends IOApp {
   ...
   */
  def handleFailure[F[_]: Applicative: Logging: Monitoring](stop: Either[Throwable, Unit]): F[ExitCode] =
    stop match {
      case Left(e) =>
        Logging[F].error(e)("Loader shutting down") *> // Making sure we always have last ERROR printed
          Monitoring[F].trackException(e) *>
          Monitoring[F].track(LoaderError.RuntimeError(e.getMessage).asLeft).as(ExitCode.Error)
      case Right(_) =>
        Applicative[F].pure(ExitCode.Success)
    }
}
