...
object FolderMonitoring {
  ...
  def sinkFolders[F[_]: Sync: Logging: AWS](input: S3.Folder, output: S3.Key): F[Unit] =
    Stream.eval(Ref.of(0)).flatMap { ref =>
      AWS[F].listS3(input, recursive = false).map(_.key)
        .map(key => s"$key\t$output")
  ...
  def check[F[_]: Monad: AWS: JDBC](loadFrom: S3.Folder, redshiftConfig: StorageTarget.Redshift): LoaderAction[F, List[AlertPayload]] =
    for {
      ...
      onlyS3Batches     <- JDBC[F].executeQueryList[S3.Folder](FoldersMinusManifest(redshiftConfig.schema, loadFrom))
      foldersWithChecks <- LoaderAction.liftF(checkShreddingComplete[F](onlyS3Batches))
        ...
        else createAlertPayload(folder, "Corrupted batch")
      }

  /** Get stream of S3 keys emitted with configured interval */
  def getOutputKeys[F[_]: Timer: Functor](folders: Config.Folders): Stream[F, S3.Key] =
    Stream.fixedRate[F](folders.period).evalMap { _ =>
      Timer[F]
        .clock
        .realTime(TimeUnit.MILLISECONDS)
        .map(Instant.ofEpochMilli)
        .map(LogTimeFormatter.format)
        .map(time => folders.staging.append("shredded").append(time).withKey("keys"))
      }
  ...
  def stream[F[_]: Sync: Timer: AWS: JDBC: Logging: Monitoring](folders: Config.Folders,
    ...
    getOutputKeys[F](folders).evalMap { outputKey =>
      val sinkAndCheck = sinkFolders[F](output, outputKey) *>
        check[F](outputKey.getParent, storage)
          .value
          .flatMap {
