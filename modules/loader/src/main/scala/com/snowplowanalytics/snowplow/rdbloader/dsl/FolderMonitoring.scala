...
object FolderMonitoring {
  ...
  private def createAlerts[F[_]: Monad: BlobStorage: Logging](folders: List[BlobStorage.Folder]): F[List[AlertMessage]] =
    folders
      .traverseFilter { folder =>
        val whenShreddingComplete =
          Logging[F]
            .warning(s"Unloaded folder: $folder")
            .as(Some(AlertMessage.FolderIsUnloaded(folder)))
            .widen[Option[AlertMessage]]
  ...
  private def alertForIncompleteShredding[F[_]: Applicative: Logging](folder: Folder): F[Option[AlertMessage]] =
    if (isProducedByBatchTransformer(folder))
      Logging[F]
        .warning(s"Incomplete shredding: $folder")
        .as(Some(AlertMessage.ShreddingIncomplete(folder)))
    else
      none.pure[F]
            ...
            if (acquired) { // The lock shouldn't be necessary with fixedDelay, but adding just in case
                  ...
                  sinkFolders[F](folders.since, folders.until, folders.transformerOutput, outputFolder).ifM(
                    for {
                      ...
                      _ <- if (alerts.isEmpty) Async[F].unit
                           else Monitoring[F].alert(AlertMessage.UnloadedFoldersDetected)
                    } yield (),
              ...
              sinkAndCheck.handleErrorWith { error =>
                failed.updateAndGet(_ + 1).flatMap { failedBefore =>
                  ...
                  val payload = AlertMessage.FailedFolderMonitoring(error, failedBefore)
