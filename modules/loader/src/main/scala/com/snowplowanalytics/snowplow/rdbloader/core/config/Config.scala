...
import com.snowplowanalytics.snowplow.rdbloader.common.config.{ConfigUtils, Region, Step}
import com.snowplowanalytics.snowplow.rdbloader.core.config.Config._

/**
  * Main config file parsed from HOCON
  *
  * @tparam D Kind of supported storage target
  */
...
object Config {
  ...
  /**
    * Case class to hold config implicits, so we can make region decoder
