...
trait Monitoring[F[_]] { self =>
   ...
   */
  def alert(message: AlertMessage): F[Unit]
...
object Monitoring {
  ...
  object AlertPayload {
    ...
    def ofAlertMessage(am: AlertMessage): AlertPayload =
      AlertPayload(Application, AlertMessage.getFolder(am), AlertMessage.getSeverity(am), AlertMessage.getString(am), Map.empty)
  ...
  def monitoringInterpreter[F[_]: Sync: Logging](
    ...
    new Monitoring[F] {
      ...
      def alert(message: AlertMessage): F[Unit] = {
        val payload = AlertPayload.ofAlertMessage(message)
