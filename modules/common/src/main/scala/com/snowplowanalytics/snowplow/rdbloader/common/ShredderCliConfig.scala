...
object ShredderCliConfig {
  ...
  val command = Command(s"${BuildInfo.name}-${BuildInfo.version}",
    "A job to prepare Snowplow enriched data to being loaded into Amazon Redshift warehouse")(shredJobConfig)
   ...
   */
  def loadConfigFrom(args: Array[String]): Either[String, ShredderCliConfig] =
