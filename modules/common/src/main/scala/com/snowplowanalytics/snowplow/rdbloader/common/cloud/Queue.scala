...
object Queue {
  ...
  trait Consumer[F[_]] {
    def read(stop: Stream[F, Boolean]): Stream[F, String]
  }
  ...
  object Consumer {
    ...
    def postProcess[F[_]: ConcurrentEffect: Timer: Logger, A](msg: A,
                                                              ack: F[Unit],
                                                              extension: Option[MessageDeadlineExtension[F]] = None): Stream[F, A] = {
