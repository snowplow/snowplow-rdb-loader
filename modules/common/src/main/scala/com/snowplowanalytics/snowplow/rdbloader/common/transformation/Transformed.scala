...
object Transformed {
  ...
  def shredEvent[F[_]: Monad: RegistryLookup: Clock](igluClient: Client[F, CJson],
    ...
    Hierarchy.fromEvent(event)
      .traverse { hierarchy =>
        ...
        fromHierarchy(tabular, igluClient.resolver, lookup)(hierarchy)
      }
   ...
   */
  private def fromHierarchy[F[_]: Monad: RegistryLookup: Clock](tabular: Boolean, resolver: => Resolver[F], lookup: LookupProperties[F])(hierarchy: Hierarchy): EitherT[F, FailureDetails.LoaderIgluError, Transformed] = {
    val vendor = hierarchy.entity.schema.vendor
    val name = hierarchy.entity.schema.name
    if (tabular) {
      EventUtils.flatten(resolver, lookup, hierarchy.entity).map { columns =>
