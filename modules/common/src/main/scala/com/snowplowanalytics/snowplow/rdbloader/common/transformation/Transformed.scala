...
case class Transformed(path: Transformed.Path, data: Transformed.Data) {
  /**
   * Convert to tuple format to be able to write it
   * to blob storage with Spark
   */
  def toTuple: Either[WideRowTuple, ShreddedTuple] = path match {
    case p: Transformed.Path.WideRow =>
      val outputType = if (p.good) "good" else "bad"
      (outputType, data.value).asLeft
    case p: Transformed.Path.Shredded =>
      val outputType = if (p.isGood) "good" else "bad"
      (outputType, p.vendor, p.name, p.format.path, p.model, data.value).asRight
  }
