...
final case class Semver(major: Int, minor: Int, patch: Int, prerelease: Option[Semver.Prerelease]) {
  override def toString: String = s"$major.$minor.$patch$prereleaseStr"

  def prereleaseStr: String = prerelease match {
    case Some(p) => p.full
    case None    => ""
  }
}
...
object Semver {
  ...
  /**
    ...
    */
  implicit private val prereleaseOrder = new PartialOrder[Option[Prerelease]] {
  ...
  implicit val prereleaseShow = new Show[Option[Prerelease]] {
  ...
  implicit val semverShow = new Show[Semver] {
    def show(version: Semver) = version.toString
  }
