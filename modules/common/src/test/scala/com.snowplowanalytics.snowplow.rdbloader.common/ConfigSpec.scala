...
class ConfigSpec extends Specification {
  "fromString" should {
    ...
    "fail if there are overlapping schema criterions" in {
      ...
      val expected = "Following schema criterions overlap in different groups (TSV, JSON, skip): " +
        "iglu:com.acme/overlap/jsonschema/1-*-*, iglu:com.acme/overlap/jsonschema/1-0-0. " +
  ...
  "Formats.multiply" should {
    "set every element from list A against all elements in list B" in {
      val expected = List((3,1), (4,1), (3,2), (4,2))
      val a = List(1,2)
      val b = List(3,4)
      val result = a.foldLeft(List.empty[(Int, Int)])(Config.Formats.multiply[Int](b))
