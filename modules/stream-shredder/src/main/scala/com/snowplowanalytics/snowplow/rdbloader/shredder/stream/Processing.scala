...
object Processing {
   ...
   */
  def getOnComplete[F[_]: Sync: Clock](compression: Compression, isTabular: SchemaKey => Boolean, root: URI, queueName: String)
    ...
    state.modify(State.updateState(find, update, _._3)).flatMap { state =>
      Completion.seal[F](compression, isTabular, root, queueName)(window, state)
    }
  }
