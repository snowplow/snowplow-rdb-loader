...
object Processing {
  ...
  def getSink[F[_]: ConcurrentEffect: ContextShift](blocker: Blocker,
    ...
    config match {
      case ShredderConfig.Output(path, _, _) =>
        ...
        generic.Partitioned.write[F, Window, SinkPath, Transformed.Data](dataSink, onComplete)
    }
  ...
  def transform[F[_]: Concurrent: Clock: Timer](iglu: Client[F, Json],
                                                ...
                                                validations: ShredderConfig.Validations): Pipe[F, Windowed[F, Parsed], Windowed[F, (SinkPath, Transformed.Data)]] = {
    _.flatMap { record =>
