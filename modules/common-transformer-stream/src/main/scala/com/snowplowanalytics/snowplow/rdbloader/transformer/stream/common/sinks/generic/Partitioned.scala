...
object Partitioned {
  ...
  /**
   * A core part of a partitioned sink It writes elements of `V` into two-level-partitioned buckets,
   * where buckets are known upfront and attached to the original `V` First bucket level is `W` - an
   * ever-growing window element, such as time period It is important to assume that every new
   * element can have same or newer window Second bucket level is `K` - a randomized key element.
   * Every `V` is associated with some `K` and `K` can repeat within `W` any amount of time and can
   * be unordered
   *
   * `D` is some local state that is accumulated as messages are added.
   *
   * Partitioning happens through [[SinkState]], which is a singleton resource associated with `W`.
   * [[SinkState]] can have (renewing over time) [[KeyedEnqueue]] that is when emitted
   *
   * @param getSink
  ...
  def write[F[_]: Concurrent, W: Order: Show, K: Show, V, D: Monoid](
    ...
    def go(s: Partitioned[F, W, K, V, D], stateOpt: Option[SinkState[W, K, V, D]]): Pull[F, (W, D), Unit] =
  ...
  /**
   * Emit a buffer ([[KeyedEnqueue]]) from existing `state` into a stream and create new
   * [[KeyedEnqueue]] attached to the same New buffer will be attached to the same window (`state`)
   *
   * @param getSink
   *   original [[Pipe]] constructor
   * @param state
  ...
  /**
   * _Potentially_ emit a buffer ([[KeyedEnqueue]]) from existing `state` into a stream and create
   * new [[KeyedEnqueue]] attached to the same New buffer will be attached to the same window
   * (`state`)
   *
   * @param getSink
   *   original [[Pipe]] constructor
