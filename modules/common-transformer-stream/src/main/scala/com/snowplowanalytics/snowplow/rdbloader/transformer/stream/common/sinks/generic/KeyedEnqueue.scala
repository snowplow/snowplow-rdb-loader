 ...
 *
 * Until "sink" it is associated with [[SinkState]]. However a single
...
object KeyedEnqueue {
  ...
  def enqueueKV[F[_]: Sync, K: Show, V](current: KeyedEnqueue[K, V], key: K, item: V): F[KeyedEnqueue[K, V]] = {
  ...
  def sink[F[_]: Concurrent, K: Show, V](enqueue: KeyedEnqueue[K, V], getSink: K => Pipe[F, V, Unit]): Stream[F, Unit] =
    Stream.iterable(enqueue.queues)
      .map { case (k, vs) =>
