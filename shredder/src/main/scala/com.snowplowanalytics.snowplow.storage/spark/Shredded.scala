...
object Shredded {
  ...
  /**
    ...
    */
  def fromHierarchy(tabular: Boolean, resolver: => Resolver[Id])(hierarchy: Hierarchy): Either[BadRow, Shredded] = {
    ...
    result.leftMap(error => BadRow.EntityShreddingError(hierarchy, error)).value
  }
}