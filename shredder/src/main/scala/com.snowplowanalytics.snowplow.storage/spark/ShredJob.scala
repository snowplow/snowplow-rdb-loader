...
object ShredJob extends SparkJob {
  ...
  def loadAndShred(client: Client[Id, Json], line: String): Either[BadRow, FingerprintedEvent] =
    for {
      ...
      fp     = event.event_fingerprint.toRight(UUID.randomUUID())
    } yield FingerprintedEvent(event, fp)
  ...
  def dedupeCrossBatch(event: FingerprintedEvent, duplicateStorage: Option[EventsManifest]): Either[BadRow, Boolean] = {
    (event, duplicateStorage) match {
      case (_, Some(storage)) =>
        try {
          ...
          case NonFatal(e) =>
            Left(BadRow.DeduplicationError(event.original, Option(e.getMessage).getOrElse(e.toString)))
        }
      case _ => Right(true)
    }
...
class ShredJob(@transient val spark: SparkSession, shredConfig: ShredJobConfig) extends Serializable {
  ...
  def run(atomicLengths: Map[String, Int]): Unit = {
    ...
    val good = common
      ...
      .groupBy { s => (s.original.event_id, s.getFingerprint) }
      .map { case (_, s) =>
        val first = s.head
